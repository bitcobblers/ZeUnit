The `SampleZeUnitClass` test in the previous [section](https://zeunit.org/docs/Quick-Start-Guide.html), was the canary in a coal mine test to make sure the testing framework is discovering and executing tests.  In this section, we will take a look at how ZeUnit makes "real world" testing easier as we tackle testing our extensible calculator class. 
