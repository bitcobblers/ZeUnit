...
ZeUnit was born from the frustrations of integration testing.  While the vast field of offerings all very in the approach, a common theme across the frameworks is the inherent misunderstanding about the target audience of these tools.
...
The sales pitch for these integration frameworks always boils down to *"Our markdown is simple enough!!!"*, but the harsh reality is that it is not and inevitably it will be an engineer that writes these tests. Now handcuffed by the *"simple"* markdown instead of being empowered by Turing-complete language.
